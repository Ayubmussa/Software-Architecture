...
function readTotalCount(headerValue: unknown, fallback: number): number {
...
async function fetchProductsRaw(params?: ProductFilters) {
  ...
  return axios.get(`${API_BASE}/api/products`, { params: query })
}

export async function fetchProducts(params?: ProductFilters): Promise<Product[]> {
  const res = await fetchProductsRaw(params)
  const raw = res.data as any[]
  return raw.map(normalizeProduct)
}

export async function fetchProductsPage(params?: ProductFilters): Promise<PaginatedResult<Product>> {
  const res = await fetchProductsRaw(params)
  const raw = res.data as any[]
  const items = raw.map(normalizeProduct)
  const total = readTotalCount(res.headers?.['x-total-count'], items.length)
  return { items, total }
}
